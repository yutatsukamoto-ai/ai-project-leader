...
> **注意**: WI-02のauto-eval.shとWI-04のauto-build.shは同じPostToolUseに掛かる。`.claude/settings.json` では同一matcherに複数hookを配列で並べられる。実行順=配列順。build → eval の順にする。
...
1. `.claude/` ディレクトリと `.claude/hooks/` を作成
2. WI-01: gate-check.sh を実装 → テスト → settings.jsonに登録
3. WI-02: auto-eval.sh を実装 → テスト → settings.jsonに追加
4. WI-03: eval-judge.md を作成 → 1ゴールデンで試行
5. WI-04: auto-build.sh を実装 → テスト → settings.jsonに追加
6. WI-05: Stopのprompt hookを settings.jsonに追加 → テスト案件で確認
7. WI-06: 残りのagentを作成
8. WI-07: CLAUDE.mdにモデルルールを追記
9. WI-08: テスト案件で一通り流して確認
10. 全受け入れ条件の通過を確認 → 完了
