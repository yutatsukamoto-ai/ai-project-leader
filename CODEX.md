...
Codexの初動は、必要な文書を用途別に読む。最初から全ドキュメント・全案件フォルダを広域探索しない。

- まず `AGENTS.md` の「最初に読む順」で作業タイプを判定する
- 決まったことの所在は `00_プロジェクト管理/構造レビュー/決定の索引.md` を入口にする
- 未決・懸念・解決ログが必要なときだけ `00_プロジェクト管理/構造レビュー/懸念マスター.md` を読む
- Skillを触るときは `20_Skills/成果物マップ.md`、`20_Skills/横断GL対応表.md`、対象Skill、関連する横断ガイドラインに絞る
- 案件名が明示されていない調査では `30_Flow/実案件/` を横断検索しない
...
- 実案件を名前だけで呼ばれたら、`30_Flow/実案件/案件インデックス.md` → 該当案件の `project-context.md` の順に読む。実案件フォルダはgitignore配下なので、実名・進捗はそこに閉じ込める。読み取りだけなら `30_Flow/現在の作業セッション.md` のアクティブ案件は変更しない
- 一般的な構造調査・grepでは、実案件名が出ていない限り `30_Flow/実案件/` を除外する。必要なら `rg --glob '!30_Flow/実案件/**' --glob '!_dist/**' --glob '!_backups/**'` のように探索範囲を絞る
...
作業完了前に、変更が「最初に読む地図」に影響するかを確認する。これは重いレビューではなく、更新漏れを防ぐ30秒チェック。
...
- 該当する親README（例: `20_Skills/90_横断/README.md`）
- 該当する横断ガイドライン（例: `40_Stock/横断ガイドライン/成果物選定ガイド.md`）

同期が必要になりやすい変更:

- Skillの追加・削除・改名・役割変更
- chain、停止ポイント、承認ゲート、起動文の変更
- Codex / Claude Code / Claude Cowork の役割分担変更
- 配布仕様、除外対象、検証コマンド、CI/smokeの変更
- `outputs/`、`project-context.md`、案件フォルダなど運用ルールの変更
- README上の件数・ステータス・現在地に影響する変更

やりすぎ防止:

- 影響がない内部修正では更新しない。
- 古い履歴文書までは直さない。正典と入口文書だけを現状に合わせる。
- 最終報告に `入口文書同期: 確認済み（更新あり/なし）` を含める。
...
- **入口文書が古いまま残る** → Skill・配布・運用ルールを変えたら、完了前に「入口文書同期チェック」を行う
