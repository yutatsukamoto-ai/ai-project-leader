...
| 運用Skill | 29（前段・立ち上げ・計画・実行・監視・終結・横断） |
...
初見で流れを掴む場合は、[前段チェーンの最小実行パス](50_サンプル成果物/md/前段_最小実行パス/) から見るのが最短です。
初回商談メモを、案件理解サマリー、課題仮説シート、計画提案書、引き継ぎ書へつなぐ流れを1本で追えます。

その他のdocx / xlsx / pptx / htmlサンプルは、[サンプル成果物インデックス](50_サンプル成果物/) にまとめています。
...
- AIプロジェクトリーダーを使ったBefore/After、短縮時間、品質差、つまずきを内部記録として残す
...
1. AIが入力品質を3行で確認し、`00_前段/01_案件理解サマリー` を作る
...
| スライド構成を作る | `このメモからスライド構成案を作って。生成前に一度止めて` |
| Codexで新規スライドを生成する | `この構成案をimage2-brand-slidesの流れでHTMLデッキ化して` |
| 既存PPTXをレビューする | `このスライドをslide-craft観点でレビューして。改善案だけ出して` |
| AIプロジェクトリーダーの効果を残す | `この案件でAIプロジェクトリーダーを使って何が変わったか、aipm-impact-logで効果記録にして` |
| 作業中の詰まりを残す | `今のつまずきをaipm-impact-logでログに残して` |
...
![成果物マップ](50_サンプル成果物/html/成果物マップ_showcase.png)
...
│   ├── 05_終結/                    # 終結報告
│   ├── 90_横断/                    # 効果記録、grill-me、業務フロー設計、スライド系3種
│   └── 99_メタ/                    # 内部テンプレ、壁打ちキックオフ、位置確認
├── 30_Flow/                        # 案件ごとの作業フォルダ
├── 40_Stock/                       # 横断ガイドライン・ナレッジ・テンプレート・案件教訓
├── 50_サンプル成果物/              # md / docx / pptx / xlsx / html サンプル
...
スライドは、Claude/Cowork側では意味設計・構成案・レビューを主に扱い、Codex併用時の新規生成は原則 `image2-brand-slides` を使います。`slide-craft` は編集可能PPTXが明示された場合、または既存PPTXレビューの補助として使います。生成画像・HTML・PDFは `30_Flow/` の正典成果物ではなく、原則 `outputs/` 配下の再生成物として扱います。

`aipm-impact-log` は、案件そのものの終結報告ではなく、AIプロジェクトリーダーを使ったことによる効果とつまずきを残す横断Skillです。フェーズ完了・案件終結・提案完了などの節目では `outputs/impact-log/` にBefore/Afterや改善材料を残し、汎用化できる学びだけを脱識別してStock化候補にします。
...
- AIプロジェクトリーダーの効果記録・つまずきログ用の横断Skillを追加済み
