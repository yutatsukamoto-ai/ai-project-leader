...
デフォルトは、参照資料を役割分担して使う `reference-informed consulting proposal style` にする。外資系コンサル系資料からは形とロジック、Retty型資料からは色階層とパーツ設計を抽象化して使う。真っ白ではない背景、資料テーマから翻訳した主色、主色の濃淡、非強調グレー、少量アクセントで階層を作る。深緑/セージは、施工・設備・技術文書レビューのような静かな提案資料に合う場合の候補であり、固定デフォルトではない。ロゴや公式サイト素材は主役にせず、資料テーマに合わせて翻訳する。
